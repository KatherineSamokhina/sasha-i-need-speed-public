...
function updateOpponents(dt) {
  for (const o of opponents) {
    ...
    // СТРАХ СКОРОСТИ (фикс Саши «гипертрасса не имеет победных
    // шансов»): раньше враг проходил повороты «по рельсам» с одним
    // и тем же сбросом, а игрока на 350 км/ч честно выносило с
    // трассы. Теперь чем быстрее враг, тем сильнее он сбрасывает
    // в дуге — как живой гонщик. До 180 км/ч ничего не меняется.
    const oKmh = o.speed / KMH;
    const speedFear = 1 + Math.max(0, (oKmh - 180) / 300);
    const curveSlow = 1 - Math.min(0.55, Math.abs(seg.curve) * 0.07 * speedFear);
